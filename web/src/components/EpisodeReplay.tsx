...
import { fetchTrainingEpisode } from '../api'
import type { TraceStep, TrainingEpisodeDetail } from '../types'
...
function surprise(step: TraceStep): number {
...
export function EpisodeReplay({ episodeId }: EpisodeReplayProps) {
  ...
  useEffect(() => {
    if (!episodeId) return
    fetchTrainingEpisode(episodeId)
      .then((payload) => {
        setError(null)
        setEpisode(payload)
  ...
  if (!episodeId) {
    return <section className="episode-replay empty">选择一个 episode 查看逐步回放。</section>
  }
  if (error) {
    return <section className="episode-replay empty error"><AlertTriangle size={16} />{error}</section>
  }
  if (!episode || !selected) {
    return <section className="episode-replay empty"><RefreshCw size={16} className="spin" />读取 episode…</section>
  }

import { Binary, Braces, BrainCircuit, Columns2, Database, Eye, GitBranch, ListTree, MousePointerClick, ScanSearch } from 'lucide-react'
...
export type DetailTab = 'trajectory' | 'input' | 'perception' | 'decision' | 'operation' | 'diff' | 'raw' | 'hypotheses' | 'imagination'
...
function JsonPanel({ value }: { value: unknown }) {
  return <div className="data-json-body"><CompactJson value={value} /></div>
}
  ...
  return (
    <section className={`event-detail ${tab === null ? 'collapsed' : ''}`}>
      <div className="detail-tabs">
        ...
        <button className={tab === 'imagination' ? 'active' : ''} onClick={() => onTab('imagination')}><Eye size={13} />想象</button>
      ...
      {tab !== null && <div className="detail-content">
        ...
        {tab === 'hypotheses' && selected && (
          <div className="hypothesis-detail-layout">
            <DecisionCard step={selected} />
            <DataSection title="逐动作变换函数表" meta={`${selected.hypotheses?.length ?? 0} HYPOTHESES`}>
              <div className="compact-table hypothesis-table">
                <div className="compact-head"><span>ACTION</span><span>VECTOR</span><span>函数</span><span>CONF</span><span>SUPPORT</span><span>ALTERNATIVES</span></div>
                {selected.hypotheses?.length ? selected.hypotheses.map((hypothesis) => (
                  <div className={`compact-row ${hypothesis.action_id === selected.action_id ? 'chosen-row' : ''}`} key={`${hypothesis.action}-${readableTransform(hypothesis.transform)}`}>
                    <b>{hypothesis.action}</b>
                    <VectorFieldOverlay spec={hypothesis.transform} compact />
                    <code>{hypothesis.readable ?? readableTransform(hypothesis.transform)}</code>
                    <strong>{Math.round(hypothesis.confidence * 100)}%</strong>
                    <span>{hypothesis.support ?? '—'}{hypothesis.total !== undefined ? `/${hypothesis.total}` : ''}</span>
                    <span>{hypothesis.alternatives?.map((alt) => `${alt.readable ?? readableTransform(alt.transform)} ${Math.round(alt.confidence * 100)}%`).join(' · ') ?? '—'}</span>
                  </div>
                )) : <div className="empty-row">本步没有 audit.v3 假设字段。</div>}
              </div>
            </DataSection>
          </div>
        )}

        {tab === 'imagination' && selected && (
          <div className="imagination-detail-layout">
            <ImaginationView step={selected} />
            <DataSection title="想象误差信号" meta={`SURPRISE ${(selected.surprise?.value ?? 0).toFixed(2)}`}>
              <JsonPanel value={{ surprise: selected.surprise, credibility: selected.credibility, error_pixels: selected.imagination?.error_pixels?.slice(0, 80) ?? [] }} />
            </DataSection>
          </div>
        )}

        {tab === 'raw' && selected && <div className="raw-event"><CompactJson value={selected} /></div>}
      </div>}
    </section>
  )
}

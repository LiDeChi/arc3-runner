import type { ActionCandidate, GameInfo, GameRun, SuiteRun, TraceStep, TransformHypothesis, TransformSpec } from './types'
...
function transformForAction(actionId: number, step: number): TransformSpec {
  if (actionId === 1) return step >= 8 ? { op: 'translate', dx: 1, dy: 0 } : { op: 'translate', dx: 0, dy: -1 }
  if (actionId === 2) return { op: 'translate', dx: 0, dy: 1 }
  if (actionId === 3) return { op: 'rotate', k: 1, pivot: 'object' }
  if (actionId === 4) return { op: 'compose', fs: [{ op: 'rotate', k: 1, pivot: 'object' }, { op: 'translate', dx: 1, dy: 0 }] }
  return { op: 'identity' }
}

function frameErrors(predicted: number[][] | null, actual: number[][]): Array<{ x: number; y: number; before: number; after: number }> {
  if (!predicted) return []
  return actual.flatMap((row, y) =>
    row.flatMap((value, x) => predicted[y]?.[x] === value ? [] : [{ x, y, before: predicted[y]?.[x] ?? 0, after: value }]),
  )
}

function hypothesesForStep(step: number, selectedActionId: number): TransformHypothesis[] {
  return [1, 2, 3, 4].map((actionId) => {
    const transform = transformForAction(actionId, step)
    const support = Math.max(0, Math.floor(step / 3) + (selectedActionId === actionId ? 1 : 0))
    const confidence = Math.min(0.96, 0.48 + support * 0.06 + (selectedActionId === actionId ? 0.18 : 0))
    return {
      action: `ACTION${actionId}`,
      action_id: actionId,
      transform,
      readable: readableTransform(transform),
      confidence: Number(confidence.toFixed(2)),
      calibrated_confidence: Number(Math.max(0.1, confidence - (step === 8 ? 0.18 : 0.07)).toFixed(2)),
      support,
      total: Math.max(1, step),
      target: 'foreground',
      alternatives: [
        { transform: { op: 'identity' }, readable: 'I', confidence: Number(Math.max(0.05, confidence - 0.22).toFixed(2)), support: Math.max(0, support - 1) },
        { transform: { op: 'mirror', axis: actionId % 2 ? 'x' : 'y' }, readable: `M(${actionId % 2 ? 'x' : 'y'})`, confidence: Number(Math.max(0.05, confidence - 0.31).toFixed(2)), support: Math.max(0, support - 2) },
      ],
    }
  })
}
...
export function buildDemoGameRun(gameId: string): GameRun {
  ...
  const steps: TraceStep[] = actionSequence.map((actionName, index) => {
    ...
    const hypotheses = hypothesesForStep(index, actionId)
    const predictedFrame = index === 0
      ? frame
      : index === 8
        ? beforeFrame
        : frame
    const predictionErrors = frameErrors(predictedFrame, frame)
    const surpriseValue = index === 0 ? 0 : Math.min(1, predictionErrors.length / Math.max(1, changed))
    const selectedHypothesis = hypotheses.find((hypothesis) => hypothesis.action_id === actionId) ?? hypotheses[0]
    const reasoning = {
    ...
    return {
      ...
      decision_gate: index === 0 ? 'reset' : index < 5 ? 'probe' : 'exploit',
      hypotheses,
      surprise: {
        value: Number(surpriseValue.toFixed(2)),
        predicted_error_pixels: predictionErrors.length,
        belief_flips: index === 8 ? ['ACTION1: T(0,-1) → T(1,0)'] : [],
      },
      credibility: {
        claimed: selectedHypothesis.confidence,
        calibrated: selectedHypothesis.calibrated_confidence ?? selectedHypothesis.confidence,
      },
      imagination: {
        before_frame: beforeFrame,
        predicted_frame: predictedFrame,
        actual_frame: frame,
        error_pixels: predictionErrors,
      },
    }
  })

...
class Rotate(Transform):
    ...
    def __post_init__(self) -> None:
        normalized = self.k % 4
        object.__setattr__(self, "k", normalized)
    ...
    def apply(
        ...
        *,
        ...
        if self.pivot == "object":
            return _to_grid(_rotate_object(array, self.k, _background(array, background_color)))
...
class ColorMap(Transform):
    ...
    def __post_init__(self) -> None:
        normalized = {int(key): int(value) for key, value in self.mapping.items()}
...
class Toggle(Transform):
    ...
    def __post_init__(self) -> None:
        normalized = tuple(sorted((int(x), int(y)) for x, y in self.cells))
...
class Compose(Transform):
    ...
    def __init__(self, *fs: Transform | Sequence[Transform]) -> None:
        if len(fs) == 1 and isinstance(fs[0], Sequence):
            transforms = tuple(fs[0])
        else:
            transforms = tuple(fs)
        if not transforms:
...
def from_spec(spec: Mapping[str, Any]) -> Transform:
    ...
    if op in ("color_map", "colormap", "c"):
        ...
        return ColorMap({int(key): int(value) for key, value in mapping.items()})
    if op in ("toggle", "g"):
        return Toggle(
